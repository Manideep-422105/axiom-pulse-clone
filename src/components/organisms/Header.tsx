...
const DepositModal = dynamic(
  () => import("../../modules/pulse/components/organisms/modals/DepositModal"),
  { ssr: false }
);
const WatchlistModal = dynamic(
  () =>
    import("../../modules/pulse/components/organisms/modals/WatchlistModal"),
  { ssr: false }
);
const NotificationsModal = dynamic(
  () =>
    import(
      "../../modules/pulse/components/organisms/modals/NotificationsModal"
    ),
  { ssr: false }
);
const WalletModal = dynamic(
  () => import("../../modules/pulse/components/organisms/modals/WalletModal"),
  { ssr: false }
);
...
const NavButton = memo<NavButtonProps>(({ label, href, active = false }) => (
  <Link href={href}>
    <button
      className={`flex flex-row h-[32px] text-nowrap px-[8px] xl:px-[14px] justify-start items-center rounded-[4px] font-medium text-[14px] transition-all duration-0 hover:duration-135 ease-in-out ${
        active
...
const IconButton = memo<IconButtonProps>(
  ({ icon: Icon, className = "", onClick }) => (
    <button
      onClick={onClick}
      className={`bg-primaryStroke hover:bg-secondaryStroke/80 flex flex-row w-[32px] h-[32px] justify-center items-center rounded-full transition-colors ${className}`}
    >
...
const Header: React.FC = () => {
  ...
  const toggleChain = useCallback(() => setIsChainOpen((prev) => !prev), []);
  const toggleNotifications = useCallback(
    () => setIsNotificationsOpen((prev) => !prev),
    []
  );
  const toggleWallet = useCallback(() => setIsWalletOpen((prev) => !prev), []);
  const openWatchlist = useCallback(() => setIsWatchlistOpen(true), []);
  const openDeposit = useCallback(() => setIsDepositModalOpen(true), []);
  const closeDeposit = useCallback(() => setIsDepositModalOpen(false), []);
  const closeWatchlist = useCallback(() => setIsWatchlistOpen(false), []);
  ...
  const handleChainSelect = useCallback((chain: ChainOption) => {
  ...
  return (
    <header className="w-full bg-background border-b border-primaryStroke font-sans z-50 relative">
        ...
        <div className="flex flex-row gap-[16px] justify-start items-center">
          ...
          <div className="flex items-center gap-[8px] sm:gap-[16px]">
            ...
            <div className="hidden sm:flex items-center gap-[8px] lg:gap-[16px]">
              <IconButton icon={RiStarLine} onClick={openWatchlist} />
      ...
      {isDepositModalOpen && (
        <DepositModal isOpen={isDepositModalOpen} onClose={closeDeposit} />
      )}
      {isWatchlistOpen && (
