...
const ToggleSwitch = memo<ToggleSwitchProps>(({ checked, onChange }) => (
  <button
    onClick={onChange}
    className={`relative rounded-full transition-all duration-200 ease-in-out overflow-hidden w-[52px] sm:w-[40px] h-[24px] sm:h-[20px] flex-shrink-0 ${
      checked
        ? "bg-primaryBlue hover:bg-primaryBlueHover"
        : "bg-secondaryStroke hover:bg-textTertiary/40"
    }`}
  >
    ...
    <div
      className={`absolute rounded-full bg-white transform transition-transform duration-200 ease-out top-[3px] sm:top-[2px] left-[2px] w-[18px] sm:w-[16px] h-[18px] sm:h-[16px] ${
        checked ? "translate-x-[30px] sm:translate-x-[20px]" : "translate-x-0"
      }`}
    ></div>
  </button>
));
...
const AlertsModal = memo<AlertsModalProps>(({ isOpen, onClose }) => {
