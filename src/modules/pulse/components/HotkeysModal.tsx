...
import { RiCloseLine, RiInformationLine, RiResetLeftLine } from "react-icons/ri";
    ...
    >
      ...
      <div className="flex flex-col flex-1 w-full overflow-y-auto">
        ...
        <div className="border-b border-secondaryStroke flex flex-col w-full p-[16px] gap-[16px]">
          ...
          <div className="flex flex-col w-full gap-[8px]">
            <div className="group flex flex-row items-center justify-between h-[32px] w-full cursor-pointer">
              <span className="text-textSecondary font-medium text-[14px] sm:text-[12px] leading-[16px]">Hotkeys</span>
              <ToggleButton active />
            </div>
            <span className="text-textTertiary text-[12px] leading-[16px] font-normal">Quick buy tokens with custom hotkeys</span>
          </div>

          {/* Toggle: Pause on Hover */}
          <div className="flex flex-col w-full gap-[8px]">
            <div className="group flex flex-row items-center justify-between h-[32px] w-full cursor-pointer">
              <span className="text-textSecondary font-medium text-[14px] sm:text-[12px] leading-[16px]">Pause live feed on Hover</span>
              <ToggleButton active />
            </div>
          </div>
          ...
          <div className="flex flex-row w-full">
            <div className="flex flex-row gap-[6px] w-full justify-start items-center bg-secondaryStroke/50 rounded-[4px] p-[8px]">
              ...
              <span className="text-[12px] text-textSecondary">Combine the Pause + Modifier + Row keys to buy tokens</span>
            </div>
          </div>
          ...
          <div className="flex flex-col w-full gap-[8px]">
            <div className="flex flex-row items-center justify-between h-[32px] w-full">
              <span className="text-textSecondary font-medium text-[14px] sm:text-[12px] leading-[16px]">Pause Key</span>
        ...
        <div className="flex flex-col w-full p-[16px] pb-[20px] gap-[16px]">
          ...
          <div className="flex flex-col gap-[12px] p-[12px] rounded-[8px] border border-secondaryStroke/50">
            <span className="text-textPrimary text-[13px] leading-[18px] font-medium">Table Modifier Keys</span>
          ...
          {/* Row Keys (1-0) */}
          <div className="flex flex-col gap-[12px] p-[12px] rounded-[8px] border border-secondaryStroke/50">
            <span className="text-textPrimary text-[13px] leading-[18px] font-medium">Row Keys</span>
            <div className="flex flex-col gap-[12px]">
              {["1", "2", "3", "4", "5", "6", "7", "8", "9", "0"].map((key, idx) => (
                <div key={key} className="flex flex-row items-center justify-between h-[32px]">
                  <div className="flex items-center gap-[8px]">
                    <span className="text-textSecondary font-medium text-[14px] sm:text-[12px] leading-[16px]">Row {idx === 9 ? 10 : idx + 1}</span>
                  </div>
                  <KeyInput value={key} />
                </div>
              ))}
      ...
      <div className="border-t border-secondaryStroke/50 flex flex-row w-full h-[68px] justify-end items-center p-[16px] pb-[20px]">
        <div className="flex flex-row flex-1 justify-start items-center">
          <button type="button" className="group bg-transparent flex flex-row h-[32px] px-[12px] gap-[8px] justify-center items-center rounded-full hover:brightness-110 transition-all duration-[150ms] cursor-pointer">
            <RiResetLeftLine className="text-[14px] text-textTertiary group-hover:text-textSecondary" />
            <span className="text-[14px] font-medium text-textSecondary">Reset</span>
          </button>
        </div>
        <div className="flex flex-row flex-1 justify-end items-center">
          <button onClick={onClose} type="button" className="bg-primaryBlue flex flex-row h-[32px] px-[12px] gap-[8px] justify-center items-center rounded-full hover:bg-primaryBlue/80 hover:brightness-110 transition-all duration-[150ms] cursor-pointer">
...
function ToggleButton({ active }: { active?: boolean }) {
  return (
    <button className="relative rounded-full overflow-hidden w-[52px] sm:w-[40px] h-[24px] sm:h-[20px] bg-primaryBlue group-hover:bg-primaryBlueHover transition-all duration-150 ease-in-out">
      ...
      <div 
        className={`absolute rounded-full bg-white transform transition-transform duration-200 ease-out top-[3px] sm:top-[2px] left-[2px] w-[18px] sm:w-[16px] h-[18px] sm:h-[16px] ${active ? 'translate-x-[30px] sm:translate-x-[20px]' : 'translate-x-0'}`}
      ></div>
    </button>
  );
}

function KeyInput({ value }: { value: string }) {
  return (
    <button type="button" className="flex items-center justify-center h-[32px] px-[12px] border-[1px] rounded-[4px] cursor-pointer transition-colors min-w-[72px] max-w-[72px] w-[72px] border-secondaryStroke hover:bg-primaryBlue/5 hover:border-primaryBlue">
      <input 
        readOnly 
...
function ModifierRow({ label, activeKey }: { label: string, activeKey: string }) {
  const keys = ["Shift", "Ctrl", "Alt", "Win"];
  return (
    <div className="flex flex-row items-center justify-between w-full min-h-[32px]">
      <span className="text-textSecondary font-medium text-[14px] sm:text-[12px] leading-[16px]">{label}</span>
