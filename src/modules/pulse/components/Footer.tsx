            ...
            <div className="relative">
              <button className="group flex items-center gap-[6px] h-[24px] pl-[8px] pr-[5px] border border-primaryStroke rounded-full hover:bg-primaryStroke/60 transition-colors">
                <div className="flex items-center gap-[4px]">
                  ...
                  <span className="text-[12px] font-medium text-textTertiary group-hover:text-textSecondary">1</span>
                </div>
                <div className="flex items-center gap-[4px]">
          ...
          <div className="hidden lg:flex items-center gap-[16px] flex-shrink-0">
             ...
             <button className="hidden lg:flex flex-row h-[24px] gap-[4px] items-center hover:brightness-110 transition-all">
                <div className="relative">
                   ...
                   <div className="relative flex h-[20px] w-[40px] rounded-full opacity-30" 
                        style={{ background: 'linear-gradient(to right, rgb(83, 211, 142) 0%, rgb(231, 140, 25) 50%, rgb(62, 154, 0) 100%)' }}>
                   </div>
                   {/* Icons Overlay */}
        ...
        <div className="flex flex-row items-center gap-[8px] flex-shrink-0 justify-end">
          ...
          <div className="hidden 2xl:flex items-center gap-[4px]">
             ...
             <button className="-mr-[8px] group flex items-center gap-[4px] h-[24px] px-2 rounded hover:bg-secondaryStroke/40 text-textTertiary transition-colors">
                ...
                <span className="text-[12px] font-normal group-hover:text-textSecondary">$50.7K</span>
             </button>
             
             {/* Gas Stat */}
             <div className="flex flex-row gap-[4px] h-[24px] items-center px-2">
                <RiGasStationLine className="text-textTertiary text-[16px]" />
                <span className="text-textTertiary text-[12px] font-normal">0.0<sub className="bottom-0">2</sub>4</span>
             </div>

             {/* Coin Stat */}
             <div className="flex flex-row gap-[4px] h-[24px] items-center px-2">
                <RiCoinLine className="text-textTertiary text-[16px]" />
                <span className="text-textTertiary text-[12px] font-normal">0.02</span>
             </div>
             <Divider />
          </div>
          ...
          {/* Docs Link */}
          <a href="#" className="hidden md:flex flex-row gap-[2px] h-[24px] px-[8px] justify-start items-center rounded-[4px] hover:bg-white/5 transition-colors">
...
function Divider() {
  return <div className="w-[1px] h-[20px] bg-primaryStroke flex-shrink-0"></div>;
}
...
function FooterLink({ icon, label, hasDot }: { icon: React.ReactNode, label: string, hasDot?: boolean }) {
  return (
    <button className="group relative flex flex-row gap-[4px] h-[24px] px-[4px] justify-start items-center rounded-[4px] hover:bg-primaryStroke/60 transition-colors cursor-pointer border border-transparent hover:border-transparent">
      ...
      <span className="text-[16px] text-textTertiary group-hover:text-textSecondary transition-colors">{icon}</span>
      <span className="text-textSecondary text-[12px] font-medium text-nowrap leading-[16px]">{label}</span>
    </button>
  );
}
...
function TickerItem({ icon, label, color, hideOn }: { icon: string, label: string, color: string, hideOn: string }) {
  return (
     <button className={`${hideOn} ${color} flex flex-row flex-shrink-0 h-[24px] px-[0px] gap-[4px] justify-start items-center hover:brightness-110 transition-all`}>
